//! Ernie 4.5 text decoder forward (phase 2).
//!
//! Mirrors `Ernie4_5ForCausalLM` in
//! `PaddlePaddle/PaddleOCR-VL-1.5/modeling_paddleocr_vl.py:636` — a stock GQA
...
/// One Ernie 4.5 decoder block: pre-RMSNorm + Attention + residual +
/// pre-RMSNorm + MLP + residual.
pub struct Ernie45DecoderLayer {
    pub input_layernorm: nn::RmsNorm,
    pub self_attn: Ernie45Attention,
...
/// Ernie 4.5 text decoder body: embedding table + N layers + final RMSNorm.
/// Does **not** include the lm_head (see [`Ernie45ForCausalLM`]).
pub struct Ernie45Model {
    pub embed_tokens: nn::Embedding,
    pub layers: Vec<Ernie45DecoderLayer>,
    pub norm: nn::RmsNorm,
        ...
        C: KeyValueCache + Default,
    {
        ...
        if cache.len() != self.layers.len() {
            return Err(Error::InvalidConfig(format!(
                "Ernie45Model::forward_from_embeds: cache len {} != num_layers {}",
                cache.len(),
                self.layers.len()
            )));
        }
...
/// Ernie 4.5 with a separate `lm_head`. The canonical PaddleOCR-VL 1.5
/// checkpoint has `tie_word_embeddings = false`, so the head is its own
/// projection from `(B, T, hidden)` → `(B, T, vocab)`.
pub struct Ernie45ForCausalLM {
    pub model: Ernie45Model,
    pub lm_head: nn::Linear,
    pub config: PaddleOcrVlConfig,
}

impl Ernie45ForCausalLM {
...
/// Build an **uninitialised** (random-weight) `Ernie45ForCausalLM` matching
/// `config`. Used by the smoke tests below; production loaders fill the
/// weights from safetensors after construction.
pub fn build_with_random_weights(config: &PaddleOcrVlConfig) -> Result<Ernie45ForCausalLM> {
    ...
    let v = config
        .vocab_size
        .ok_or_else(|| Error::InvalidConfig("vocab_size missing".to_string()))?;
...
mod tests {
    ...
    fn tiny_config() -> PaddleOcrVlConfig {
        serde_json::from_str::<PaddleOcrVlConfig>(
            r#"{
    ...
    fn last_logits_shape_is_vocab_wide() {
        ...
        let last_only = model
            ...
            .unwrap();
        assert_eq!(last_only.shape(), &[1, cfg.vocab_size.unwrap()]);
    }
